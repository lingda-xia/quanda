...
class SetLabel extends StatelessWidget {
  ...
  /// 逻辑层
  final logic = Get.put(SetLabelLogic());

  /// 数据层
  final state = Get.find<SetLabelLogic>().state;
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        ...
        actions: [
          /// 确定按钮
          GetBuilder<SetLabelLogic>(builder: (e) {
            return Visibility(
              visible: state.textContent != "",
              child: GestureDetector(
                onTap: () => logic.editLabel(),
                child: Container(
      ...
      body: GetBuilder<SetLabelLogic>(builder: (e) {
        /// 通讯录列表  自定义widget
        return AddressList(
          /// 头部widget
          fixedHead: fixedHead(),
